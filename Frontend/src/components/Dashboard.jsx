...
export default function Dashboard() {
  ...
  useEffect(() => {
    // AuthContext already validates the token and refreshes the user profile
    // on boot, so calling refreshUser() here too would trigger a second
    // re-render of the nav/avatar moments after the page settles - visible
    // as a flicker. Credits are refreshed explicitly after actions that
    // change them (in Marketplace/Calculator), which is where it matters.
    load();
    ...
    <AppShell>
      ...
      <section className="stats-grid">
        {cards.map((c, i) => (
          <button
            type="button"
            key={c.label}
            className={`card stat-card spotlight${activeCard === i ? " active" : ""}`}
      ...
      {data.byDomain.length > 0 && (
        <>
          ...
          <section className="panel spotlight" onMouseMove={spotlightMove}>
