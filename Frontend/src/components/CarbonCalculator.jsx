...
// Client-side preview factors (grams CO2e per passenger-km) — mirror the
// backend's emissionFactors.json for an instant readout. The authoritative
// figure is always computed server-side with the verified table on submit.
const PREVIEW_FACTORS = {
  private_car: 171, two_wheeler: 72, auto_rickshaw: 108,
  public_transit: 41, shared_ride: 171, electric_vehicle: 60, walk_cycle: 0,
};
...
export default function CarbonCalculator() {
  ...
  const preview = useMemo(() => {
    if (domain === "transportation" && transport.subtype !== "shared_ride") {
      ...
      const saved = Math.max(0, km * PREVIEW_FACTORS.private_car - emitted);
  ...
  return (
    <AppShell wide>
      ...
      <div className="calc-layout">
        <form className="panel" onSubmit={isSharedRide ? undefined : submit}>
          {domain === "transportation" && (
            <>
                ...
                {(config.transportModes || []).map((m) => (
                  <button key={m.key} type="button"
                    ...
                    <span className="calc-mode-icon">
                      {{ private_car: "🚗", two_wheeler: "🛵", auto_rickshaw: "🛺", public_transit: "🚌",
              ...
              {transport.subtype === "public_transit" && (
                <>
        ...
        <div className="panel calc-readout">
          ...
          <p className="field-hint" style={{ marginTop: 18 }}>
            Preview only — the exact figure is calculated when you log it, using the verified
            emission factor table and any GPS/verification checks.
          </p>
        </div>
      </div>
      ...
      {history.length === 0 ? (
        <div className="empty-state"><h3>Nothing logged yet</h3><p>Your history will appear here.</p></div>
      ) : (
