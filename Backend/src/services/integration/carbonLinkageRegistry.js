...
const calcRideShare = (txn) => {
  ...
  const saved = Math.max(0, km * transportFactor("petrol_car") - perPerson);
...
const calcSharedErrand = (txn) => {
  ...
  const emissions = km * transportFactor("petrol_car");
  const saved = Math.round(km * transportFactor("petrol_car"));
